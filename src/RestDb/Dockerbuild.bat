...
docker buildx build -f Dockerfile --platform linux/amd64,linux/arm64/v8 --tag jchristn77/restdb:%1 --tag jchristn77/restdb:latest --load .

IF "%2" NEQ "1" GOTO :Done

ECHO.
ECHO Pushing images...
docker push jchristn77/restdb:%1
docker push jchristn77/restdb:latest
